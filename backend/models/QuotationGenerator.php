...
public function generateExcelQuotation($orderData, $order_id) {
    try {
        ...
        $sheet->setCellValue('D' . $rowStart, 'Unit Price (AUD)');
        $sheet->setCellValue('E' . $rowStart, 'Total Price (AUD)');
        ...
        foreach ($items as $index => $item) {
            ...
            if (isset($item['selections'])) {
                ...
                // If no specs were generated, try direct fields
                if (empty($specs) && !empty($productDetails)) {
                    $specs .= 'Type: ' . ($productDetails['product_type'] ?? '') . "\n";
                    $specs .= 'Material: ' . ($productDetails['material'] ?? '') . "\n";
                    $specs .= 'Connection: ' . ($productDetails['flange_type'] ?? '') . "\n";
                    $specs .= 'Size: ' . ($productDetails['flange_size'] ?? '') . "\n";
                    $specs .= 'Pressure: ' . ($productDetails['operating_pressure'] ?? '') . "\n";
                } else if (empty($specs) && isset($item['product']) && is_array($item['product'])) {
                    $specs .= 'Type: ' . ($item['product']['type'] ?? '') . "\n";
                    $specs .= 'Material: ' . ($item['product']['material'] ?? '') . "\n";
                    $specs .= 'Connection: ' . ($item['product']['connection'] ?? '') . "\n";
                    $specs .= 'Size: ' . ($item['product']['size'] ?? '') . "\n";
                    $specs .= 'Pressure: ' . ($item['product']['pressure'] ?? '') . "\n";
                }
            }
            ...
            $sheet->setCellValue('B' . $rowCurrent, $quantity); // Use actual quantity
            $sheet->setCellValue('C' . $rowCurrent, $description);
            $sheet->setCellValue('D' . $rowCurrent, ''); // Price column empty
            $sheet->setCellValue('E' . $rowCurrent, ''); // Total column empty
